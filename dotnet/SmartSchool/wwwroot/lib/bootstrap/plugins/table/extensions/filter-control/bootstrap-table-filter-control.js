...
(function($) {
    ...
    var setValues = function(that) {
        ...
        if (that.options.valuesFilterControl.length > 0) {
            header.find(searchControls).each(function(index, ele) {
                field = $(this).closest("[data-field]").data("field");
                result = $.grep(that.options.valuesFilterControl,
                    function(valueObj) {
    ...
    var initFilterSelectControls = function(that) {
        ...
        $.each(that.header.fields,
            function(j, field) {
                ...
                if (isColumnSearchableViaSelect(column) &&
                    ...
                    for (var i = 0; i < z; i++) {
                        ...
                        var fieldValue = data[i][field],
                            formattedValue = $.fn.bootstrapTable.utils.calculateObjectValue(that.header,
    ...
    var createControls = function(that, header) {
            ...
            function(i, column) {
                ...
                if (!column.filterControl) {
                    ...
                    if (column.searchable && that.options.filterTemplate[nameControl]) {
                        ...
                        isVisible = "visible";
                        html.push(that.options.filterTemplate[nameControl](that,
                            column.field,
                            isVisible,
                ...
                if (column.filterData !== undefined && column.filterData.toLowerCase() !== "column") {
                    ...
                    if (filterDataType !== null) {
                        filterDataSource =
                            column.filterData.substring(column.filterData.indexOf(":") + 1, column.filterData.length);
                        selectControl = $(".bootstrap-table-filter-control-" + escapeID(column.field));
        ...
        if (addedFilterControl) {
            header.off("keyup", "input").on("keyup",
                "input",
                function(event) {
            ...
            header.off("change", "select").on("change",
                "select",
                function(event) {
                    clearTimeout(timeoutId);
                    timeoutId = setTimeout(function() {
            ...
            header.off("mouseup", "input").on("mouseup",
                "input",
                function(event) {
                    var $input = $(this),
                        oldValue = $input.val();
                    ...
                    function(i, column) {
                        if (column.filterControl !== undefined && column.filterControl.toLowerCase() === "datepicker") {
                            header.find(".date-filter-control.bootstrap-table-filter-control-" + column.field)
                                ...
                                .on("changeDate",
                                    function(e) {
                                        $(sprintf(".%s", e.currentTarget.classList.toString().split(" ").join(".")))
    ...
    BootstrapTable.prototype.clearFilterControl = function() {
        if (this.options.filterControl && this.options.filterShowClear) {
            ...
            $.each(that.options.valuesFilterControl,
                function(i, item) {
                    item.value = "";
                });
            ...
            if (that.options.sortName !== table.data("sortName") ||
                that.options.sortOrder !== table.data("sortOrder")) {
                var sorter =
                    header.find(sprintf('[data-field="%s"]', $(controls[0]).closest("table").data("sortName")));
                if (sorter.length > 0) {

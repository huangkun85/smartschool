...
(function($) {
    ...
    BootstrapTable.prototype.initHeader = function() {
        ...
        table.closest(".fixed-table-container").find(".fixed-table-body")
        ...
        function render_sticky_header(event) {
            ...
            var e_end = $("#" + anchor_end_id).offset().top -
                header_height -
                $("#" + table_header_id).css("height").replace("px", "");
            // show sticky when top anchor touches header, and when bottom anchor not exceeded
            if (t > e && t <= e_end) {
                // ensure clone and source column widths are the same
                $.each(that.$stickyHeader.find("tr").eq(0).find("th"),
                    function(index, item) {
                        $(item).css("min-width",
                            $("#" + table_header_id + " tr").eq(0).find("th").eq(index).css("width"));
                    });
        ...
        function match_position_x(event) {
            ...
            $("#" + sticky_header_container_id + " thead").parent()

...
(function() {
    ...
    angular.module("bsTable", [])
        ...
        .directive("bsTableControl",
            [
                "uiBsTables", function(uiBsTables) {
                    ...
                    $(document)
                        .on("post-header.bs.table",
                            ...
                            function(evt) { // bootstrap-table calls .off('scroll') in initHeader so reattach here
                        ...
                        .on("sort.bs.table",
                            CONTAINER_SELECTOR + " table",
                            function(evt, sortName, sortOrder) {
                                var bsTable = getBsTable(evt.target);
                                if (!bsTable) return;
                                var state = bsTable.$s.bsTableControl.state;
                                bsTable.$s.$applyAsync(function() {
                        ...
                        .on("page-change.bs.table",
                            CONTAINER_SELECTOR + " table",
                            function(evt, pageNumber, pageSize) {
                                var bsTable = getBsTable(evt.target);
                                if (!bsTable) return;
                                var state = bsTable.$s.bsTableControl.state;
                                bsTable.$s.$applyAsync(function() {
                        ...
                        .on("focus blur",
                            CONTAINER_SELECTOR + " " + SEARCH_SELECTOR,
                            function(evt) {
                                var bsTable = getBsTable(evt.target);
                                if (!bsTable) return;
                                var state = bsTable.$s.bsTableControl.state;
                    ...
                    return {
                        ...
                        link: function($s, $el) {
                            ...
                            $s.$watch("bsTableControl.options",
                                function(options) {
                                    ...
                                    if ("searchHasFocus" in state)
                                        $el.closest(CONTAINER_SELECTOR).find(SEARCH_SELECTOR)
                                            .focus(); // $el gets detached so have to recompute whole chain
                                },
                                true);
                            $s.$watch("bsTableControl.state",
                                function(state) {

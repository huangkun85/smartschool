...
(function($) {
    ...
    var showSortModal = function(that) {
        ...
        if (!$(_id).hasClass("modal")) {
            ...
            sModal += '                     <button type="button" class="btn btn-default" data-dismiss="modal">' +
                that.options.formatCancel() +
                "</button>";
            sModal += '                     <button type="button" class="btn btn-primary">' +
            ...
            that.$sortModal.off("click", "#add").on("click",
                "#add",
                function() {
            ...
            that.$sortModal.off("click", "#delete").on("click",
                "#delete",
                function() {
                    var total = that.$sortModal.find(".multi-sort-name:first option").length,
                        current = that.$sortModal.find("tbody tr").length;
            ...
            that.$sortModal.off("click", ".btn-primary").on("click",
                ".btn-primary",
                function() {
                    var $rows = that.$sortModal.find("tbody > tr"),
                    ...
                    that.options.sortPriority = $.map($rows,
                        function(row) {
                            var $row = $(row),
                    ...
                    if (results.length > 0) {
                        if ($alert.length === 0) {
                            $alert = '<div class="alert alert-danger" role="alert"><strong>' +
    ...
    BootstrapTable.prototype.initToolbar = function() {
        ...
        if (this.options.showMultiSort) {
            ...
            if (!$multiSortBtn.length) {
                $multiSortBtn = '  <button class="multi-sort btn btn-default' +
    ...
    BootstrapTable.prototype.addLevel = function(index, sortPriority) {
        ...
        $.each(this.options.formatSortOrders(),
        ...
        for (var i = 0; i < headers.length; i++) {
            for (var c = 0; c < that.options.sortPriority.length; c++) {
                if ($(headers[i]).data("field") === that.options.sortPriority[c].sortName) {
                    $(headers[i]).find(".sortable").removeClass("desc asc")
                        .addClass(that.options.sortPriority[c].sortOrder);
                }
            }
        }
    };
